...
- **反爬纪律（必须遵守）**：脚本已内置"下载失败等 30 秒只重试 1 次"。若仍失败，告知用户间隔几分钟后再试，**不要连续手动重跑**——短时间重复请求会被抖音临时风控，越试越糟
- 音频缓存：下载成功的音频按链接缓存在 `~/.config/video-transcript/cache/`，同一链接重跑（如 ASR 失败后重试）直接命中缓存、不请求抖音；需强制重新下载时删除对应缓存文件
- 纯自用工具，提取内容用于个人学习沉淀
- 视频本身不落盘，只有音频缓存和 Markdown 笔记留存
